...
const MOCK_CONSULTATIONS = [
  ...
  },
  {
    ...
    mode: "in-person",
    status: "completed",
    diagnosis: "Heart Failure Management",
    icd: ["I50.9"],
    billing: 2500,
  },
];
    ...
    async function fetchData() {
      try {
        ...
        if (data.consultations?.length > 0) {
          const mapped = data.consultations.map(
            (c: Record<string, unknown>, i: number) => {
              ...
              const emr =
                Array.isArray(c.emr_entries) && c.emr_entries.length > 0
                  ...
                  : null;
              const billing =
                Array.isArray(c.billing) && c.billing.length > 0
                  ? c.billing[0]
                  : null;
              return {
                ...
                time: new Date(c.started_at as string).toLocaleTimeString(
                  "en-IN",
                  { hour: "numeric", minute: "2-digit", hour12: true },
                ),
                duration: c.duration_sec
                  ? `${Math.round(Number(c.duration_sec) / 60)} min`
                  : "—",
                type: (c.consultation_type ?? c.type ?? "general") as string,
                mode: (c.mode ?? "in-person") as string,
                status: (c.status ?? "active") as string,
                diagnosis: (emr?.chief_complaint ??
                  c.chief_complaint ??
                  "—") as string,
                icd: ((emr?.icd_codes ?? []) as Array<{ code: string }>).map(
                  (ic) => ic.code,
                ),
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span
                    className={`inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full ${statusCfg.color}`}
                  >
                    <StatusIcon className="w-3 h-3" />
                    {statusCfg.label}
                  </span>
                  <span className="text-xs font-medium text-[var(--foreground)]">
                    {c.billing > 0 ? `₹${c.billing.toLocaleString()}` : "—"}
                  </span>
                ...
                {isExpanded && (
                  ...
                  >
                    <div className="grid grid-cols-4 gap-4 text-xs">
                      <div>
                        <p className="text-[10px] text-[var(--foreground-subtle)] uppercase mb-0.5">
                          Duration
                        </p>
