...
const MOCK_AUDIT_ENTRIES = Array.from({ length: 25 }, (_, i) => {
  ...
  return {
    ...
    previous_hash:
      i === 0 ? "0000000000000000" : mockHash(i - 1).substring(0, 16),
    payload: {
      details:
        eventType === "SAFETY_ALERT_TRIGGERED"
          ? "Drug interaction: Metformin + Contrast Dye"
          : eventType === "ICD_CODE_MAPPED"
            ? "E11.9 — Type 2 Diabetes Mellitus without complications"
            : eventType === "PRESCRIPTION_ADDED"
              ? "Metformin 500mg — Twice daily"
              : eventType === "CONSENT_RECORDED"
                ? "Voice consent recorded, OTP eSign completed"
        ...
        if (data.entries?.length > 0) {
          const mapped = data.entries.map(
            (e: Record<string, unknown>, i: number) => ({
              ...
              previous_hash: (
                (e.previous_hash ?? "0000000000000000") as string
              ).substring(0, 16),
              payload: (e.payload ?? { details: "No details" }) as {
                details: string;
              },
            }),
          );
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
        ...
        <CardContent className="p-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold text-green-400">
              Blockchain-like Audit Chain — Integrity Verified
            </p>
            <p className="text-[10px] text-[var(--foreground-muted)] mt-0.5">
              All {auditEntries.length} events are cryptographically chained
              with SHA-256 hashes. No tampering detected.
            </p>
          </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by actor, patient, event, or ID..."
      ...
      <div className="space-y-1">
        {filtered.map((entry, i) => {
          ...
          const cfg =
            EVENT_CONFIG[entry.event_type] || EVENT_CONFIG.CONSULTATION_STARTED;
            ...
            >
              ...
              >
                ...
                <div className="flex flex-col items-center gap-0.5 shrink-0">
                  <div
                    className={`w-6 h-6 rounded-md flex items-center justify-center ${cfg.color.replace("text-", "bg-").replace("-400", "-500/10")}`}
                  >
                ...
                <div className="text-right shrink-0">
                  <p className="text-[9px] font-mono text-[var(--foreground-subtle)]">
                ...
                {isExpanded ? (
                  <ChevronDown className="w-3.5 h-3.5 text-[var(--foreground-subtle)] shrink-0" />
                ) : (
                ...
                >
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <p className="capitalize text-[var(--foreground)]">
                        {entry.actor_role}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 pt-1 border-t border-[var(--border-subtle)]">
                    <div>
                      <p className="text-[8px] text-[var(--foreground-subtle)] uppercase">
                        Current Hash
                      </p>
                      <p className="text-[10px] font-mono text-green-400">
                        {entry.hash}
