...
const AVAILABLE_DOCTORS = [
  ...
  },
  {
    ...
    name: "Dr. Neha Patel",
    department: "General Medicine",
    available_slots: ["9:00 AM", "11:30 AM", "2:30 PM", "5:00 PM"],
  },
  {
    id: "d3",
  ...
  },
  {
    ...
    available_slots: ["9:30 AM", "12:00 PM", "3:00 PM"],
  },
];
...
export function PatientAppointmentsClient({
    ...
    try {
      ...
      if (data.upcoming?.length > 0 || data.past?.length > 0) {
        const mapAppt = (a: Record<string, unknown>) => ({
          ...
          department: ((a.doctor as { department?: string })?.department ??
            "") as string,
          date: new Date(a.date as string).toLocaleDateString("en-IN", {
            month: "short",
            day: "numeric",
            year: "numeric",
          }),
          time: (a.time_slot ?? "") as string,
          type: (a.type ?? "general") as
            | "general"
            | "followup"
            | "emergency"
            | "teleconsult",
          mode: (a.mode ?? "in-person") as "in-person" | "teleconsult",
          status: (a.status ?? "confirmed") as
            | "confirmed"
            | "completed"
            | "cancelled",
  ...
  const selectedDoctorData = AVAILABLE_DOCTORS.find(
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant={
                      appt.mode === "teleconsult" ? "default" : "secondary"
                    }
                    className="text-[8px]"
                  >
              ...
              <div className="text-right shrink-0">
                ...
                <div
                  className={`inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[9px] font-medium ${getStatusColor(appt.status)}`}
                >
