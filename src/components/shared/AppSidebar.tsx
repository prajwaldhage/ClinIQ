  ...
  receptionist: [
    {
      ...
      href: "/receptionist",
      icon: ClipboardList,
      description: "Queue & registration",
    },
  ],
};
...
export function AppSidebar({ user, alertCount = 0, onSignOut }: SidebarProps) {
  ...
  return (
    <motion.aside
      ...
      className="relative flex flex-col h-screen bg-[var(--sidebar)] border-r border-[var(--sidebar-border)] overflow-hidden shrink-0"
    >
      {/* ─── Header ────────────────────────────────────────────── */}
      <div className="flex items-center gap-3 px-4 py-4 border-b border-[var(--sidebar-border)]">
        <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-[#4A90E2] to-[#3CC2C2] shrink-0">
      ...
      <div className="px-3 py-3 border-b border-[var(--sidebar-border)]">
        <div className="flex items-center gap-2.5">
          <div className="relative shrink-0">
            ...
            <span
              className={cn(
                "absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-[var(--sidebar)]",
                roleMeta.dot,
              )}
            />
          </div>
          ...
          return (
            <Link key={item.href} href={item.href}>
              <motion.div
                ...
                whileTap={{ scale: 0.97 }}
                className={cn(
                  "flex items-center gap-3 px-2.5 py-2 rounded-lg text-xs font-medium transition-all duration-200 cursor-pointer group",
                  isActive
      ...
      <div className="px-2 py-3 space-y-0.5">
        <button
          className={cn(
            "flex w-full items-center gap-3 px-2.5 py-2 rounded-lg text-xs text-[var(--text-secondary)] hover:bg-[var(--surface-elevated)] hover:text-[var(--text-primary)] transition-all duration-200",
          )}
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.span
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="flex-1 text-left text-xs text-[var(--text-secondary)]"
              >
                Theme
              </motion.span>
            )}
        ...
        <button
          onClick={onSignOut}
          className="flex w-full items-center gap-3 px-2.5 py-2 rounded-lg text-xs text-red-400/70 hover:bg-red-500/10 hover:text-red-400 transition-all duration-200"
      ...
      <button
        ...
        className="absolute -right-3 top-[72px] flex items-center justify-center w-6 h-6 rounded-full bg-[var(--surface)] border border-[var(--border)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--primary)] transition-all duration-200 z-10 shadow-sm"
      >
        {collapsed ? (
