...
const MOCK_CONSENTS = [
  ...
  {
    id: "c2",
    purpose: "Government Epidemiology",
    description:
      "Share aggregated data with public health authorities for disease tracking",
    enabled: true,
  },
  {
    id: "c3",
    ...
    description:
      "Allow automated sharing of reports with your insurance provider",
    enabled: false,
  },
  {
    id: "c4",
    purpose: "WhatsApp Notifications",
    description: "Receive appointment reminders and reports via WhatsApp",
    enabled: true,
  },
];
      ...
      try {
        ...
        if (data.profile) {
          ...
          setProfile({
            ...
            chronic_conditions:
              p.chronic_conditions ?? MOCK_PROFILE.chronic_conditions,
            emergency_contact:
              p.emergency_contact ?? MOCK_PROFILE.emergency_contact,
            language_pref: (p.language_pref ?? "en") as
              | "en"
              | "hi"
              | "hinglish",
          });
          if (
            p.family_history &&
            Array.isArray(p.family_history) &&
        ...
        if (data.consents?.length > 0) {
          setConsents(
            data.consents.map((c: Record<string, unknown>) => ({
              id: c.id as string,
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-[var(--foreground-subtle)] uppercase tracking-wider">
                    Full Name
                  </label>
                  {isEditing ? (
                    <input
              ...
              <div className="flex gap-2">
                {(["en", "hi", "hinglish"] as const).map((lang) => (
                  <button
                    key={lang}
                    onClick={() =>
                      setProfile({ ...profile, language_pref: lang })
                    }
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
                ...
                {member.conditions.length > 0 && (
                  <div className="flex gap-1">
                    {member.conditions.some((c) =>
                      profile.chronic_conditions.includes(c),
                    ) && (
            ...
            <p className="text-[9px] text-[var(--foreground-subtle)] pt-1">
              ...
              Family history helps identify hereditary risk factors. Shared
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
              ...
              >
                ...
                <button
                  ...
                  className={`w-10 h-5 rounded-full transition-colors relative shrink-0 ${
                    consent.enabled ? "bg-green-600" : "bg-[var(--surface)]"
                  }`}
                >
                  <motion.div
            ...
            <div className="flex items-start gap-2 pt-2 text-[9px] text-[var(--foreground-subtle)]">
              ...
              Your data is protected with zero-knowledge proofs. Personal
              identifying information is never shared directly. HIPAA & ABDM
              compliant.
            </div>
          </CardContent>
        </Card>
